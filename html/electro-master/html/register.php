...
if (!isset($_POST['username'], $_POST['primer_apellido'], $_POST['segundo_apellido'], $_POST['email'], $_POST['password'], $_POST['tel'], $_POST['domicilio'])) {
...
if (empty($_POST['username']) || empty($_POST['primer_apellido']) || empty($_POST['segundo_apellido'])  || empty($_POST['email'])  || empty($_POST['password'])  || empty($_POST['tel']) || empty($_POST['domicilio'])) {
  ...
  exit('¡El username no es valido!');
	...
	if (isset($_POST['submit'])) {
		...
		if ($revisar !== false ) {
			...
			$insertar = $db->query("INSERT INTO clientes(image) VALUES ('$imgContenido')");
			if ($insertar) {
				echo "Archivo subido";
			} else{
				echo "ha fallado";
...
if ($stmt = $con->prepare('SELECT id_cliente, contraseña FROM clientes WHERE nombre_cliente = ?')) {
  ...
  $stmt->bind_param('s', $_POST['id_cliente']);
  $stmt->execute();
  $stmt->store_result();
  ...
  if ($stmt->num_rows > 0) {
    ...
    if ($stmt = $con->prepare('INSERT INTO clientes (nombre_cliente, primer_apellido, segundo_apellido, correo, contraseña, telefono, domicilio ) VALUES (?, ?, ?, ?, ?, ?, ?)')) {
      ...
      $stmt->bind_param('sssssss', $_POST['username'], $_POST['primer_apellido'], $_POST['segundo_apellido'], $_POST['email'], $_POST['password'], $_POST['tel'], $_POST['domicilio'] );
      $stmt->execute();
...
?>
                    ...
                    <a href="inicio-producto.php"><input class="btn btn-primary" value="Aceptar" ></a>
                    </form>
                </div>
            </div>        
        </div>
